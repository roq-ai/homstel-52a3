...
import { getAdvisorById } from 'apiSdk/advisors';
import { AdvisorInterface } from 'interfaces/advisor';

function AdvisorViewPage() {
  const { hasAccess } = useAuthorizationApi();
  const router = useRouter();
  const id = router.query.id as string;
  const { data, error, isLoading, mutate } = useSWR<AdvisorInterface>(
    () => (id ? `/advisors/${id}` : null),
    () =>
      getAdvisorById(id, {
        relations: ['user'],
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('advisor', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/advisors/edit/${id}`} passHref legacyBehavior>
                    <Button
