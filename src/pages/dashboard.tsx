...
import { getAdvisors } from 'apiSdk/advisors';
...
import { ExperienceListPage as ShowCaseList } from 'pages/experiences';
...
function DashboardPage() {
  ...
  const advisorStat = useSWR<PaginatedInterface<AdvisorInterface>>(
    () => `/advisors`,
    () => getAdvisors({ limit: 1, offset: 0 }),
  );

  const tripStat = useSWR<PaginatedInterface<TripInterface>>(
    () => `/trips`,
    () => getTrips({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Advisors',
      result: advisorStat?.data?.totalCount,
      isLoading: advisorStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Trips',
      result: tripStat?.data?.totalCount,
      isLoading: tripStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`experiences`} passHref legacyBehavior>
                  <Button
